...
print_results("Exercise 1 results", vpn_docs)
...
print_results("Exercise 2 results", high_docs)
...
student_docs = collection.get(
    where_document={"$contains": "student"}
)

print_results("Exercise 3A results", student_docs)

student_no_password_docs = collection.get(
    where_document={
        "$and": [
            {"$contains": "student"},
...
print_results("Exercise 3B results", student_no_password_docs)
...
print_results("Exercise 4 filtered query", filtered_query, show_distances=True)

unfiltered_query = collection.query(
    query_texts=["how do I print documents on campus"],
    n_results=5,
    include=["documents", "metadatas", "distances"]
)

print_results("Exercise 4 unfiltered query", unfiltered_query, show_distances=True)